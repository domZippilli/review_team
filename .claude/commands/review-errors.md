You are running a focused **error handling** review.
...
Use the `review-errors` agent (via the Task tool with `subagent_type: "review-errors"`) to perform the review. In the prompt, include:
- The full diff, changed file list, and any PR context
- Instruction to follow its expertise file
- **The output file path**: `Write your findings to: reviews/<review-id>/error-handling.md`

The agent will write its findings directly to disk.

## Step 3: Present findings

Read the findings from `reviews/<review-id>/error-handling.md` and present them to the user. Group by severity (CRITICAL first, then WARNING, then INFO).

Tell the user: **"Review saved to `reviews/<review-id>/error-handling.md`"**
